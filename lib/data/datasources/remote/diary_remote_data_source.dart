...
import '../../models/diary/diary_details_model.dart';
...
class DiaryRemoteDataSourceImpl extends DiaryRemoteDataSource {
  ...
  @override
  Future<ResponseModel<DiaryDetailsModel>> fetchDiaryDetails(
      FetchDiaryDetailParams params, String token) async {
    try {
      final response = await client.get(
          Uri.parse('$baseUrl/svc/diaryDetail/${params.diaryId}'),
          headers: {
            'Content-Type': 'application/json',
            'Authorization': 'Bearer $token',
          });
      ...
      if (response.statusCode == 200) {
        return responseData['data'] == null
            ? ResponseModel.fromJsonWithoutData(responseData)
            : ResponseModel.fromJsonMap(
                responseData, DiaryDetailsModel.fromJson);
      } else {
        throw ServerException();
      }
    } catch (e) {
